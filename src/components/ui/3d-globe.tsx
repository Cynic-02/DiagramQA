    ...
    <group ref={groupRef} visible={isVisible}>
      ...
      <group ref={imageGroupRef} position={topPosition}>
        <Html transform center sprite distanceFactor={10} style={{ pointerEvents: isVisible ? "auto" : "none", opacity: isVisible ? 1 : 0, transition: "opacity 0.15s ease-out" }}>
          <div
            className={cn("cursor-pointer overflow-hidden rounded-full bg-card shadow-lg transition-transform duration-200", hovered && "scale-125 shadow-xl ring-1 ring-foreground/50")}
...
interface SceneProps { markers: GlobeMarker[]; config: Required<Globe3DConfig>; onMarkerClick?: (marker: GlobeMarker) => void; onMarkerHover?: (marker: GlobeMarker | null) => void; }
...
function LoadingFallback() {
  return <Html center><div className="flex shrink-0 flex-col items-center gap-3"><span className="inline-block shrink-0 text-sm text-muted-foreground">Loading globe...</span></div></Html>;
}
