...
/**
 * Shuffle — per-letter "slot machine" text reveal (React Bits).
 *
 * Ported as-is from reactbits.dev with two adjustments to fit this repo:
 *   1. TypeScript props (matching every other component in this folder).
 *   2. No hardcoded font/size/color in Shuffle.css — those come from
 *      whatever heading/paragraph classes the caller already applies via
 *      `className`, so the component composes with the site's theme
 *      tokens instead of fighting them.
 *
 * Requires gsap's (free, bundled since 3.13) SplitText plugin + ScrollTrigger,
 * driven through @gsap/react's useGSAP for correct scoping/cleanup in React.
 */
